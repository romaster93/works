...
h_conv11 = tf.nn.relu(tf.nn.conv2d_transpose(h_conv10, W_conv11, output_shape=[1,4,4,512], strides=[1, 2, 2, 1], padding='SAME'))      #4
...
##컨벌루션 진행
W_convp1 = tf.Variable(tf.truncated_normal(shape=[3, 3, 512, 512], stddev=5e-2))
...
h_conv12 = tf.nn.relu(tf.nn.conv2d_transpose(h_conv11, W_conv12, output_shape = [1,32//4,32//4,256], strides=[1, 2, 2, 1], padding='SAME'))
...
##컨벌루션 진행
W_convp2 = tf.Variable(tf.truncated_normal(shape=[3, 3, 256, 256], stddev=5e-2))
...
h_conv13 = tf.nn.relu(tf.nn.conv2d_transpose(h_convp2, W_conv13, output_shape = [1,32//2,32//2,128], strides=[1, 2, 2, 1], padding='SAME'))
...
##컨벌루션 진행
W_convp3 = tf.Variable(tf.truncated_normal(shape=[3, 3, 128, 128], stddev=5e-2))
...
h_conv14 = tf.nn.relu(tf.nn.conv2d_transpose(h_conv11, W_conv12, output_shape = [1,32,32,64], strides=[1, 2, 2, 1], padding='SAME'))
...
##컨벌루션 진행
W_convp4 = tf.Variable(tf.truncated_normal(shape=[3, 3, 64, 64], stddev=5e-2))
h_convp4 = tf.nn.relu(tf.nn.conv2d(cat_14, W_convp4, strides=[1, 1, 1, 1], padding='SAME'))

##손실함수 활성화함수

loss= tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=y, logits=h_convp4))

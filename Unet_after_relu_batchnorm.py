...
##잘라붙이기
cat_11 = tf.concat([h_conv8, bn_conv11], axis=-1)
...
##잘라붙이기
cat_12 = tf.concat([h_conv6, bn_conv13], axis=-1)
...
##잘라붙이기
cat_13 = tf.concat([h_conv4, bn_conv15], axis=-1)
...
##잘라붙이기
cat_14 = tf.concat([h_conv2, bn_conv17], axis=-1)
...
loss = tf.reduce_mean(tf.losses.mean_squared_error(labels=y, predictions=bn_conv18))
